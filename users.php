                ...
                <div class="profile-content">
                    ...
                    <div class="tab-content p-0">
                        ...
                        <div class="tab-pane fade in active show" id="profile-friends">
                            <h4 class="m-t-0 m-b-20">Friend Suggested <span id="count_users"></span></h4>
                            <!-- begin row -->
                            <div class="row row-space-2" id="friend-list-2424wqd">
                                <!-- begin col-6 -->
...
<script type="module">
    ...
    (function() {
        const fetchAllUsers = async () => {
            try {
                let response = await fetch(`<?= CONTROLLER ?>/UserController.php?fetch-all-users=${true}`);
                response = await response.json();
                if (response.status == 400 || response.status == 500) {
                    errorAlert('Sorry An error occurred while processing your request. please try again later.');
                }
                if (response.users.length > 0) {
                    let friendlist = document.getElementById("friend-list-2424wqd");
                    let list = response.users.map((user) => {
                        // https://bootdey.com/img/Content/avatar/avatar2.png
                        return `<div class="col-md-6 mb-2" id="container-${user.id}">
                           <div class="p-10 bg-white ">
                              <div class="media media-xs overflow-visible d-flex flex-row justify-content-around">
                                 <a class="media-left" href="javascript:;">
                                 <img src="${user.profile == 'default-profile.png' ? './public/images/default-profile.png' : user.profile}" alt="" class="media-object img-circle">
                                 </a>
                                 <div class="media-body valign-middle my-auto">
                                    <b class="text-inverse">${user.first_name} ${user.last_name}</b>
                                 </div>
                                 <div class="media-body valign-middle text-right overflow-visible">
                                    ...
                                    id="dropdownMenuButton1" data-bs-toggle="dropdown" aria-expanded="false">
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuButton1">
                                            <li><a class="dropdown-item add-friend-btn" data-key-reference=${user.id} href="#">Add Friend</a></li>
                                            <li><a class="dropdown-item" href="./profile/user.php?user=${user.id}">View Profile</a></li>
                    ...
                    document.getElementById("count_users").textContent = response.users.length;
                    addClickBtn();
                }
            } catch {
                errorAlert('Sorry An error occurred while processing your request. please try again later.');
        ...
        const addClickBtn = () => {
            document.querySelectorAll(".add-friend-btn").forEach((btn) => {
                btn.addEventListener("click", async (e) => {
                    e.preventDefault();
                    try {
                        let response = await addFriend(e.target.dataset.keyReference);
                        if (response.status == 200) {
                            successAlert("Friend Request Send!");
                            document.getElementById(`container-${e.target.dataset.keyReference}`).remove();
        ...
        const addFriend = async (ref) => {
            let form = new FormData();
            form.append('ref', ref);
            form.append('add-friend-request', ref);
            //try {
            let response = await fetch(`./controller/UserController.php`, {
                method: 'POST',
                body: form
            });
            response = await response.json();
